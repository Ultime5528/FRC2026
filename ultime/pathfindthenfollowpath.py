        ...
        if self._targetPath is not None:
            ...
            if self._shouldFlipPath():
                ...
                self._goalEndState = GoalEndState(
                    self._goalEndState.velocity, self._targetPose.rotation()
                )

        if currentPose.translation().distance(self._targetPose.translation()) < 0.5:
            self._output(
                ChassisSpeeds(), DriveFeedforwards.zeros(self._robotConfig.numModules)
            )
            self._finish = True
        else:
    ...
    def execute(self):
        ...
        skipUpdates = (
            ...
            and currentPose.translation().distance(
                self._currentTrajectory.getEndState().pose.translation()
            )
            < 2.0
        )
        ...
        if not skipUpdates and Pathfinding.isNewPathAvailable():
            ...
            if self._currentPath is not None:
                ...
                while closestState2Idx < len(self._currentTrajectory.getStates()) - 1:
                    closest2Dist = (
                        self._currentTrajectory.getState(closestState2Idx)
                        .pose.translation()
                        .distance(currentPose.translation())
                    )
                    nextDist = (
                        self._currentTrajectory.getState(closestState2Idx + 1)
                        .pose.translation()
                        .distance(currentPose.translation())
                    )
                ...
                d = closestState1.pose.translation().distance(
                    closestState2.pose.translation()
                )
                t = (
                    currentPose.translation().distance(closestState1.pose.translation())
                ) / d
                t = min(1.0, max(0.0, t))
    ...
    def __init__(
        ...
        *requirements: Subsystem
        ...
        def buildJoinCommand() -> Command:
            ...
            startFieldSpeeds = ChassisSpeeds.fromRobotRelativeSpeeds(
            ...
            endWaypoint = Pose2d(
                goal_path.getPoint(0).position, goal_path.getInitialHeading()
            )
            ...
            endState = GoalEndState(
                pathfinding_constraints.maxVelocityMps, startPose.rotation()
            )
            if goal_path.getIdealStartingState() is not None:
                ...
                endState = GoalEndState(
                    goal_path.getIdealStartingState().velocity, endRot
                )

            joinPath = PathPlannerPath(
                PathPlannerPath.waypointsFromPoses(
                    [Pose2d(startPose.translation(), startHeading), endWaypoint]
                ),
                pathfinding_constraints,
                IdealStartingState(
                    hypot(startSpeeds.vx, startSpeeds.vy), startPose.rotation()
                ),
                endState,
            )
