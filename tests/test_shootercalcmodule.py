...
def _test_ShooterCalcModule_common(
    ...
    rotation_to_align = shooter_calc_module.getRotationToAlignWithTarget()

    transform = Transform2d(0.0, 0.0, rotation_to_align)
    ...
    assert rotation_to_align.radians() == approx(angle_solution, abs=0.005)
...
def test_zero_angles():
    ...
    for angle in angles:
        ...
        rotation_to_align_simple = computeRobotRotationToAlignSimple(
            rotated_shooter_pose3d, rotated_target
        )
        assert rotation_to_align_simple.radians() == approx(0.0, abs=0.0001)
...
def _test_counterclockwise_common(added_angle_to_target: float, sign_of_angle):
    ...
    for angle in angles:
        ...
        assert rotation_to_align_simple.radians() * sign_of_angle > 0.0
